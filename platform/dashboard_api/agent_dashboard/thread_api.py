...
_SURFACED_SOURCES: tuple[str, ...] = ("dashboard", "schedule")
...
async def _resolve_run_email(login: str, profile: dict[str, Any]) -> str | None:
    """Email recorded on a run when the session did not supply one."""
    del login
...
def _thread_owner_login(metadata: dict[str, Any]) -> str | None:
    owner = metadata.get("owner_id") or metadata.get("github_login")
...
async def list_dashboard_threads(
    ...
    searches: list[dict[str, Any]] = (
        [{}] if include_all else [{"owner_id": login}, {"github_login": login}]
    )
...
async def _build_dashboard_configurable(
    ...
    *,
    ...
    configurable: dict[str, Any] = {
        ...
        "user_email": email or await _resolve_run_email(login, profile),
        "auth_provider": auth_provider,
        "actor_id": actor_id or login,
    }
