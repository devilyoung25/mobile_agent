...
def set_sandbox_factory(factory) -> None:
    """Register the platform's sandbox factory (injected by the app layer)."""
    global _SANDBOX_FACTORY
    _SANDBOX_FACTORY = factory


def create_sandbox(sandbox_id=None):
    if _SANDBOX_FACTORY is None:
        raise RuntimeError("sandbox factory not registered; call set_sandbox_factory() first")
    return _SANDBOX_FACTORY(sandbox_id)
